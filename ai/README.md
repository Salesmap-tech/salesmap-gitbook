---
description: >-
  본 문서는 AI에게 첨부하여 세일즈맵 API 관련 질문을 할 수 있도록 정리한 문서입니다. 내용의 정확성을 보장하지 않으며, 정확한 사항은
  아래 개발자용 문서를 참고해주시거나, 채널톡으로 문의해 주세요.
---
...
> **문서 기준일: 2026-07-31.** 이 레퍼런스는 세일즈맵 개발팀이 API를 개발·수정할 때 갱신되며, 최신본은 <https://docs.salesmap.kr/developers/api-reference/ai#api> 에 게시됩니다.
...
- [통합 검색 · 연결관계 (Search & Association)](#통합-검색--연결관계-search--association)
...
> **참고:** top-level `description`, `status`는 저장되지 않습니다(무시됨). `description` 필드는 상품에 없습니다. **상태는 생성 시 지정할 수 없으며 항상 기본값 `active`로 생성됩니다.** 유형·담당자 등은 `fieldList`로 넣습니다.
...
> **참고:** `dealId`와 `leadId`는 **정확히 하나**만 보냅니다. 둘 다 생략하면 `400`과 `[refine]: dealId 또는 leadId를 입력해주세요.`, 둘 다 보내면 `400`과 `[refine]: dealId와 leadId 중 하나만 입력해 주세요.`를 반환합니다.
> **참고:** 견적 구성 상품은 `quoteProductList`로 보냅니다. 구 필드명 `itemList`를 보내면 400 없이 **조용히 무시**되어 상품 없는 빈 견적서가 생성되므로 주의합니다.
