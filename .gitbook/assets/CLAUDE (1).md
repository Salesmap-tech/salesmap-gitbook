...
> **문서 기준일: 2026-06-26.** 이 레퍼런스는 세일즈맵 개발팀이 API를 개발·수정할 때 갱신되며, 최신본은 <https://docs.salesmap.kr/developers/api-reference/ai#api> 에 게시됩니다.
...
- [오브젝트 모델](#오브젝트-모델) · [작업별 빠른 찾기](#작업별-빠른-찾기-task--api) · [히스토리 vs 액티비티](#히스토리-vs-액티비티) · [기본 정보](#기본-정보) · [공통 응답 형식](#공통-응답-형식) · [응답 키 주의](#응답-키-주의-위치마다-다른-키) · [페이지네이션](#페이지네이션) · [에러 형식](#에러-형식) · [필드 값 쓰기 (fieldList)](#필드-값-쓰기-fieldlist) · [엔드포인트 인덱스](#엔드포인트-인덱스)

**엔드포인트**
- [고객 (People)](#고객-people)
- [회사 (Organization)](#회사-organization)
- [딜 (Deal)](#딜-deal)
- [리드 (Lead)](#리드-lead)
- [커스텀 오브젝트 (Custom Object)](#커스텀-오브젝트-custom-object)
- [견적서 · 상품 · 파이프라인 (Quote / Product / Pipeline)](#견적서--상품--파이프라인-quote--product--pipeline)
- [통합 검색 · 연관관계 (Search & Association)](#통합-검색--연관관계-search--association)
- [필드 · 파일 · 이메일 (Field / File / Email)](#필드--파일--이메일-field--file--email)
- [시퀀스 · 웹폼 · TODO · 메모 (Sequence / WebForm / Todo / Memo)](#시퀀스--웹폼--todo--메모-sequence--webform--todo--memo)
- [사용자 · 팀 (User / Team)](#사용자--팀-user--team)

**[웹훅 (Webhook)](#웹훅-webhook)**

**부록**
- [삭제 API 요약](#삭제-api-요약)
- [읽기전용 시스템 필드](#읽기전용-시스템-필드)
- [베스트 프랙티스](#베스트-프랙티스)
...
| 견적서 생성 / 딜·리드 견적 조회 | `POST /v2/quote` / `GET /v2/{deal|lead}/{id}/quote` |
...
| `email` | body | string | 선택 | 이메일 |
| `phone` | body | string | 선택 | 전화번호 |
| `ownerId` | body | string(UUID) | 선택 | 담당자 유저 ID |
| `organizationId` | body | string(UUID) | 선택 | 소속 회사 ID |
| `fieldList` | body | array `[{name, <값키>}]` | 선택 | 커스텀 필드 값. 값 키는 필드 타입에 따라 다릅니다("필드 값 쓰기" 참조) |
...
| memo | body | string | 선택 | 메모 |
| expectedCloseDate | body | string(ISO8601) | 선택 | 수주 예정일. `마감일`은 status가 Won/Lost일 때만 반영됩니다. |
| fieldList | body | array | 선택 | 커스텀 필드. `[{name, stringValue\|numberValue\|booleanValue\|...}]`. `금액`은 넣지 않습니다(위 참고). |

> **참고:** 검증 순서는 ① name/pipelineId/pipelineStageId/status 누락 검사 → ② peopleId|organizationId refine → ③ `금액` fieldList refine → ④ 파이프라인/단계 존재 검사(`파이프라인 단계를 찾을 수 없습니다.`) 입니다.
...
> **참고:** `dealId` 또는 `leadId` 중 하나는 반드시 보내야 합니다. 둘 다 생략하면 `400 Bad Request`와 `[refine]: dealId 또는 leadId를 입력해주세요.`를 반환합니다. 둘 다 함께 보낼 수도 있습니다.
> **참고:** 존재하지 않는 `dealId`/`leadId`를 보내면 `400 Bad Request`와 `dealId의 대상을 찾을 수 없습니다.` / `leadId의 대상을 찾을 수 없습니다.` 메시지를 반환합니다(이 경우 `reason`은 문자열입니다). 검증 순서는 `name` → `dealId/leadId` 존재 여부 → 나머지입니다.
...
**요청 파라미터**
없습니다. 페이지네이션도 없으며 전체를 한 번에 반환합니다.
...
> **참고:** 파이프라인/단계 식별자 키는 `_id`입니다(다른 오브젝트의 `id`/`RecordId`와 다름). 파싱 시 `_id`로 읽으세요. 이 엔드포인트는 페이지네이션이 없어 `nextCursor`를 반환하지 않으며, 응답 전체가 모든 파이프라인입니다. 또한 `name`/`index`는 영문 키로 제공됩니다(커스텀 필드 평탄화 없음).
> **참고:** 딜/리드 생성·수정 시 파이프라인은 body 파라미터 `pipelineId`(=`_id`)와 `pipelineStageId`(단계 `_id`)로 지정합니다. `pipelineStageId`를 변경할 때는 반드시 `pipelineId`와 함께 보내야 합니다(딜 섹션 참조).
...
| `…filters[].value` | body | string \| number \| array | 조건부 | `EXISTS`/`NOT_EXISTS`에서만 생략할 수 있고, 그 외 연산자에서는 필수입니다. 빈 문자열 `""`은 허용되지 않습니다. `IN`/`NOT_IN`/`DATE_BETWEEN`은 배열로 전달합니다. |
...
| 400 | Bad Request | `Operator "GT" on field "금액" requires a numeric value.` | 숫자 연산자에 문자열 value |
| 400 | Bad Request | `["[filterGroupList,0,filters,0,value]: value is required for this operator"]` | EXISTS/NOT_EXISTS 외인데 value 누락 |
...
`data.memo`는 노트 레코드 객체입니다. 단건 응답의 필드는 목록 레코드에서 `cursorId`와 `유형`을 제외한 `{ id, htmlBody, text, dealId, leadId, peopleId, organizationId, productId, quoteId, todoId, parentId, ownerId, updatedAt, createdAt }`입니다. 본문은 `htmlBody`·`text`로, 작성자는 `ownerId`로 제공됩니다.
