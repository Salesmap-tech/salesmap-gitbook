...
* 계산식 문법이 올바르지 않거나 현재 필드 생성 대상이 아닌 오브젝트의 필드를 참조하면 `400` 오류가 반환됩니다. 예를 들어 고객 필드 생성 요청에서 `{{회사.필드명}}`처럼 회사 필드를 참조할 수 없습니다.
...
| Status | 상황                                          |
| ------ | ------------------------------------------- |
| 400    | 요청 형식이 올바르지 않거나, 지원하지 않는 필드 타입 조합입니다. 계산식 문법이 올바르지 않거나 대상이 다른 오브젝트의 필드를 참조한 경우도 포함됩니다. |
| 401    | 인증에 실패했습니다.                                 |
